<script lang="ts">
	...
	type Props = {
		trigger: Visualization['elements']['horizontalTrigger'];
		bg: string;
	};

	let { trigger, bg }: Props = $props();
...
<button {...$trigger} use:trigger style:background-color={bg} aria-label="Adjust A and !A ratio"
></button>
